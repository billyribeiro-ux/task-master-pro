...
<PageShell title="Projects" description="Manage your projects and boards">
	{#snippet actions()}
		<button
			...
			class="bg-brand-600 hover:bg-brand-700 active:bg-brand-800 inline-flex min-h-11 items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-white transition"
		>
			<svg
	...
	{#if data.projects.length === 0}
		<div
			class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 py-16 dark:border-gray-700"
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
			...
			<button
				onclick={() => (showCreateModal = true)}
				class="bg-brand-600 hover:bg-brand-700 inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-white transition"
			>
				Create Project
			</button>
		</div>
			...
			{#each data.projects as project (project.id)}
				...
				>
					<div class="mb-3 flex items-center justify-between">
						<div
							class="bg-brand-50 text-brand-600 dark:bg-brand-950 dark:text-brand-400 flex h-9 w-9 items-center justify-center rounded-lg"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								class="h-5 w-5"
					...
					<h3
						class="group-hover:text-brand-600 dark:group-hover:text-brand-400 text-sm font-semibold text-gray-900 dark:text-white"
					>
						{project.name}
					</h3>
			...
			>
				<div>
					...
					<input
						...
						class="focus:border-brand-500 focus:ring-brand-500/20 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-400 transition focus:ring-2 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white"
