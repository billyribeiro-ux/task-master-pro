<script lang="ts">
	...
	let maxCompletionVal = $derived(
		data.completionTrend.length > 0 ? Math.max(...data.completionTrend.map((d) => d.total)) : 0
	);
</script>
							...
							<div class="h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
								<div
									...
									style="width: {totalTasks > 0
										...
										: 0}%; background-color: {statusColors[item.status] ?? '#9ca3af'}"
								></div>
							</div>
						</div>
					{/each}
						...
						<div>
							...
							<div class="h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
								<div
									...
									style="width: {totalTasks > 0
										? (item.total / totalTasks) * 100
										: 0}%; background-color: {priorityColors[item.priority] ?? '#9ca3af'}"
								></div>
							</div>
						</div>
					{/each}
			...
			{:else}
				<div class="flex items-end gap-1" style="height: 120px">
					{#each data.completionTrend as day (day.date)}
						<div
							class="bg-brand-500 hover:bg-brand-600 flex-1 rounded-t transition-all"
						...
						<div>
							<div class="mb-1 flex items-center justify-between text-xs">
								...
								<span class="text-gray-500 dark:text-gray-400"
									>{project.completedTasks}/{project.totalTasks} ({pct}%)</span
								>
							</div>
							<div class="h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
								<div
