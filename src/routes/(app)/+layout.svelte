...
<div class="flex h-screen overflow-hidden" class:dark={theme.resolved === 'dark'}>
	...
	<!-- Sidebar -->
	<aside
		class="fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-gray-200 bg-white transition-transform duration-200 dark:border-gray-800 dark:bg-gray-900 lg:static lg:z-auto {mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} {sidebarOpen ? 'lg:w-64' : 'lg:w-16'}"
	>
		<!-- Sidebar header -->
		...
		<nav class="flex-1 space-y-1 overflow-y-auto p-3">
			<a
				href="/dashboard"
				class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800"
			>
		...
		<div class="hidden border-t border-gray-200 p-3 dark:border-gray-800 lg:block">
			<button
				onclick={() => (sidebarOpen = !sidebarOpen)}
				class="flex w-full items-center justify-center rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800"
	...
	<div class="flex flex-1 flex-col overflow-hidden">
		...
		<header class="flex h-14 shrink-0 items-center justify-between border-b border-gray-200 bg-white px-4 dark:border-gray-800 dark:bg-gray-900">
			<div class="flex items-center gap-3">
				<!-- Mobile menu button -->
				<button
					class="rounded-lg p-1.5 text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 lg:hidden"
				...
				<div class="relative hidden sm:block">
					...
					<input
						...
						class="w-64 rounded-lg border border-gray-200 bg-gray-50 py-1.5 pl-9 pr-3 text-sm text-gray-900 placeholder-gray-400 transition focus:border-brand-500 focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500 dark:focus:bg-gray-800"
					/>
				</div>
			</div>
				...
				<div class="relative">
					...
					>
						{#if data.user.avatarUrl}
							<img src={data.user.avatarUrl} alt={data.user.name} class="h-7 w-7 rounded-full" />
						{:else}
							<div class="flex h-7 w-7 items-center justify-center rounded-full bg-brand-100 text-xs font-semibold text-brand-700 dark:bg-brand-900 dark:text-brand-300">
								{data.user.name.charAt(0).toUpperCase()}
							</div>
						{/if}
						<span class="hidden text-sm font-medium text-gray-700 dark:text-gray-300 sm:block">{data.user.name}</span>
					</button>
					...
					{#if userMenuOpen}
						...
						>
							...
							<a href="/settings" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700" role="menuitem">Settings</a>
							<a href="/settings/billing" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700" role="menuitem">Billing</a>
							<div class="border-t border-gray-100 dark:border-gray-700">
								<form method="POST" action="/logout">
									<button type="submit" class="block w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-gray-50 dark:text-red-400 dark:hover:bg-gray-700" role="menuitem">
										Sign Out
									</button>
								</form>
							</div>
