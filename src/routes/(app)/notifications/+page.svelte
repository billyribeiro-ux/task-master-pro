...
<PageShell title="Notifications">
	{#snippet actions()}
		{#if data.notifications.some((n) => !n.isRead)}
			<button onclick={markAllRead} class="text-brand-600 hover:text-brand-500 text-sm font-medium">
				Mark all as read
			</button>
		{/if}
	{/snippet}
	...
	{:else}
		...
		>
			{#each data.notifications as notification (notification.id)}
				<div
					class="flex items-start gap-3 px-5 py-4 transition {notification.isRead
						? ''
						: 'bg-brand-50/50 dark:bg-brand-950/20'}"
				>
					<div
						class="mt-0.5 h-2 w-2 shrink-0 rounded-full {notification.isRead
							? 'bg-transparent'
							: 'bg-brand-500'}"
					></div>
					<div class="flex-1">
